...
# How much the zoom will be incremented/decremented relatively using the mouse wheel
export var mouse_zoom_factor: Vector2 = Vector2(0.75, 0.75)
...
func _process(_delta) -> void:
	...
	if not stop_on_limit and events.size() == 0:
		...
		if not position.is_equal_approx(last_pos):
			emit_signal("redraw_requested")
	...
	if _zoom != Vector2.ZERO:
		set_position(lerp(position, _position, _delta * zoom_speed))
		set_zoom(lerp(zoom, _zoom, _delta * zoom_speed))
#		position = lerp(position, _position, _delta * zoom_speed)
#		zoom = lerp(zoom, _zoom, _delta * zoom_speed)
		if zoom.is_equal_approx(_zoom):
			_zoom = Vector2.ZERO
		...
		if events.size() > 1:
			...
			if abs(pinch_distance - last_pinch_distance) > zoom_sensitivity:
				...
				if zoom_at_point:
					...
					zoom_at(new_zoom, (p1 + p2) / 2, true)
				else:
					# Otherwise, just updates de camera's zoom
					zoom_at(new_zoom, position, true)
	...
	# If the mouse events is set to be handled
		...
		if event is InputEventMouseButton and event.is_pressed():
			...
			if event.get_button_index() == BUTTON_WHEEL_UP:
				if zoom_at_point:
					zoom_at(zoom * mouse_zoom_factor, event.position)
				else:
					zoom_at(zoom * mouse_zoom_factor, position)
...
func zoom_at(new_zoom: Vector2, point: Vector2, touch = false) -> void:
	...
	if ((zoom_diff.x > 0 and new_zoom.x < max_zoom + zoom_diff.x)
			...
			if touch:
				...
				_zoom = new_zoom
				_position = position - (point * zoom_diff)
